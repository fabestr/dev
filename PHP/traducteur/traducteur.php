...
function translate($word, $tab, $direction)
{
	if ($direction == 'toEnglish') {
		if(array_key_exists($word,$tab) == true)
		{
			echo $tab[$word];
		}
	} else {
		if (in_array($word,$tab) == true){
			echo array_search($word,$tab);
		}else{
		echo 'Ce mot n\'est pas dans ce dictionnaire';
		}
	}
}
